...
| Test strategy | Unit tests only, fully mocked (no real Qdrant/bifrost) | 16-hour timebox; integration tests add risk |
...
# Pre-download embedding model at build time (~420MB, avoids cold-start download)
...
**`requirements.txt` (exact pins, latest secure versions as of Apr 2026):**
```
streamlit==1.57.0
